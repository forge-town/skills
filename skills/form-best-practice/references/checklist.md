...
- **特征**：
  - `formControl` 由所属页面 slice 通过 `createFormControl<T>()` 创建并持有（见 `store-best-practice` §三 module-level singleton 允许清单）
...
- **配套规则**：满足 `store-best-practice` §4.6（`handle*` 只由 JSX 消费）—— `handleFormSubmit` 通过 `<form onSubmit={form.handleSubmit(handleFormSubmit)}>` 绑定，禁止组件内 JS 主动调用
...
- [ ] ✅（范式 B 适用）`formControl` 实例由 slice 通过 `createFormControl<T>()` 创建并持有，组件用 `useForm({ formControl })` 接管
  - ✅ 正确示例：
    ```ts
    // slice 内（store-best-practice §三 允许 createFormControl 这种 module-level 工厂）
