...
- [ ] ✅ 非基础 UI 组件、页面和业务模块只直接使用 `div` 原生标签作为布局容器
- [ ] ✅ `button`、`input`、`select`、`textarea`、`a`、`form`、`label`、`img`、表格和文本语义标签均由职责对应的基础组件封装
- [ ] ✅ 没有通过别名、字符串标签名、`React.createElement` 或中间包装绕过原生 DOM 边界
- [ ] ✅ 基础组件文件只封装自身职责对应的原生元素，不把例外扩散给调用方
- [ ] ✅ 每项判断均记录最终可呈现结构或行为证据；无法证明时标记 `evidence-required`，不得当作 pass 或 skip
...
- [ ] ❌ 不存在业务组件直接使用 `<form>`、`<a>`、`<img>`、`<table>`、`<p>` 或标题标签的情况
