...
## SlidePresentation State Management — Required Pattern

The current slide index drives navigation dots, the slide counter, and the "next/previous" math for keyboard / touch / dot navigation. Getting this wrong produces a **silent race condition** that looks like "keyboard nav is broken" — pressing → / Space jumps once, then appears stuck or jumps to the wrong slide.

### The Bug You Will Write If You Don't Read This

Naively, the controller reads `scrollY` in a scroll handler to derive `currentIndex`:

```javascript
// ❌ WRONG — derive currentIndex from live scrollY
window.addEventListener("scroll", () => {
  this.currentIndex = Math.round(window.scrollY / window.innerHeight);
  // ... update dots, counter, progress ...
});

goTo(i) {
  this.slides[i].scrollIntoView({ behavior: "smooth" });
  // currentIndex updates "eventually" via the scroll handler above
}
```

**Why it fails:**
1. User presses → → `goTo(currentIndex + 1)` → `scrollIntoView({ behavior: "smooth" })` starts an 800ms animation
2. While animating, the scroll handler fires continuously, reading `scrollY` mid-animation and rounding it to **stale indices**
3. User presses → again before animation completes → `currentIndex + 1` uses the **stale value**, so the deck either freezes on the current slide or skips erratically

The bug is invisible during slow scroll-wheel use (state settles between actions), so it's easy to ship.

### The Fix — Single Source of Truth + Settled-Only Sync

```javascript
class SlidePresentation {
  // ... constructor sets up nav dots, IO, keyboard, touch, scroll tracking ...

  /* The ONE place that mutates currentIndex + UI. Synchronous, no scroll reads. */
  setCurrent(i) {
    const clamped = Math.max(0, Math.min(this.slides.length - 1, i));
    this.currentIndex = clamped;
    this.navDots.forEach((d, idx) => d.classList.toggle("active", idx === clamped));
    this.counterCurrent.textContent = String(clamped + 1).padStart(2, "0");
  }

  /* Update state BEFORE starting smooth scroll so rapid keypresses use the
     new index, not one read back from a still-animating scroll. */
  goTo(i) {
    const clamped = Math.max(0, Math.min(this.slides.length - 1, i));
  ...
  /* Scroll handler ONLY updates the progress bar (cheap, no state). */
  setupScrollTracking() {
    let scrollEndTimer = null;
    window.addEventListener("scroll", () => {
      this.updateProgress();
      // Debounce: only sync index from scroll AFTER the user stops scrolling.
      // This handles mouse-wheel / trackpad scrolling that doesn't go through goTo().
      clearTimeout(scrollEndTimer);
      scrollEndTimer = setTimeout(() => this.syncFromScroll(), 120);
    }, { passive: true });
    this.updateProgress();
  }

  updateProgress() {
    const total = (this.slides.length - 1) * window.innerHeight;
    const pct = total > 0 ? Math.min(100, (window.scrollY / total) * 100) : 0;
    this.progressBar.style.width = pct + "%";
  }

  /* Safe: only called after scroll settles for 120ms. */
  syncFromScroll() {
    const idx = Math.min(this.slides.length - 1, Math.round(window.scrollY / window.innerHeight));
...
After generating, check the JS for these:

- [ ] There is exactly ONE function that writes to `this.currentIndex` (e.g. `setCurrent`). Other code paths call it.
- [ ] `goTo()` calls `setCurrent()` **before** triggering `scrollIntoView`.
- [ ] The `scroll` event listener does **not** synchronously update `currentIndex` or nav-dot active state.
- [ ] Index sync from scroll position is debounced (≥ 100ms) so it only fires after motion stops.

If any of these are missing, keyboard nav will appear to "stutter" or "skip" — even though every individual event handler looks correct in isolation.
