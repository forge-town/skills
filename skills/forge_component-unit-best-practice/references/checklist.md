...
- [ ] 1.4 文件夹内包含 `ComponentName.spec.tsx` 行为测试文件
...
## 单元测试（ComponentName.spec.tsx）
- [ ] 3.1 测试文件与组件同目录
- [ ] 3.2 使用 Vitest + Testing Library，测试文件使用 `.spec.tsx`
...
- [ ] 4.4 遵循 [forge_storybook-best-practice](../../forge_storybook-best-practice/SKILL.md) 规范
...
- [ ] ❌ 不存在一个文件夹内包含多个组件的情况
- [ ] ❌ 不存在使用 `.test.tsx` 替代 `.spec.tsx` 或将测试/故事文件放在单独目录的情况
